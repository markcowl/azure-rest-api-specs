...
function resolvedLocation(location, sourceTraceLevel) {
...
function resolveNamespaceLocation(finding) {
 ...
 */
export function resolveHeadSourceLocations(findings, headProgram) {
    for (const finding of findings) {
        const diff = finding.diff;
        // Only process findings where head type is missing
        if (diff.headType || diff.headSourceLocation)
            continue;
        if (!diff.baseType || diff.baseType.kind !== "ModelProperty")
            continue;
        const prop = diff.baseType;
        const propName = prop.name;
        // Use the node's parent to get the SOURCE model name (not the projected model)
        const modelName = prop.node?.parent?.id?.sv ?? prop.model?.name;
        if (!modelName)
            continue;
        const headModel = findModelFromOrigin(headProgram, finding.diff.origin?.declarationPath) ??
...
function findModelFromOrigin(program, declarationPath) {
    if (!declarationPath) {
        return undefined;
    }
    return (findModelByQualifiedPath(program, declarationPath) ??
 ...
 */
function findModelInProgram(program, modelName) {
    // Walk the global namespace and all sub-namespaces
    const globalNs = program.getGlobalNamespaceType();
    return findModelInNamespace(globalNs, modelName);
}
function findModelInServiceNamespace(program, serviceNamespace, modelName) {
    if (!serviceNamespace) {
        return undefined;
    }
...
function findMatchingNamespace(program, namespace) {
    const namespacePath = getNamespacePath(namespace);
    let current = program.getGlobalNamespaceType();
...
function findModelByQualifiedPath(program, qualifiedPath) {
    if (!qualifiedPath) {
        return undefined;
    }
...
function findModelInNamespace(ns, modelName) {
