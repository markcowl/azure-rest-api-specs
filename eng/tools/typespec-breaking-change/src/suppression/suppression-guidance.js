 ...
 */
export function formatSuppressionDiff(finding) {
    const decoratorName = finding.phase === "same-version" ? "@approvedUnversionedChange" : "@approvedBreakingChange";
    const hasDirectHeadLocation = finding.diff.headSourceTraceLevel === "direct" ||
 ...
 */
function getPropertyPath(origin) {
 ...
 */
function buildRemovedPropertyDiff(origin, decorator) {
    const parts = origin.declarationPath.split(".");
...
function getLineAtPos(text, pos) {
