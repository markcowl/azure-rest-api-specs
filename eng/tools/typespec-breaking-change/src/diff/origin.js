 ...
 */
function traceToTemplateSourceProperty(prop) {
    return (traceToTemplateSourceModel(prop.model, prop.name, new Set()) ??
        traceToTemplateArgumentProperty(prop.model, prop.name, new Set()));
}
function traceToTemplateSourceModel(model, propertyName, seen) {
    ...
    for (const source of model.sourceModels ?? []) {
        ...
        const nested = traceToTemplateSourceModel(sourceModel, propertyName, seen) ??
...
function traceToTemplateArgumentProperty(model, propertyName, seen) {
    const args = (model?.templateMapper?.args ?? []);
    if (!args)
        return undefined;
    for (const arg of args) {
        if (arg.kind !== "Model" || seen.has(arg))
            continue;
        seen.add(arg);
        ...
        const nested = traceToTemplateSourceModel(arg, propertyName, seen) ??
